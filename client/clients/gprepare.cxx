...
void usage() {
      ...
      fprintf (stderr, "          \t              : 0  = File is beeing prepared");
      fprintf (stderr, "          \t              : -1 = File couln't be prepared ");
...
int main(int argc, char* argv[]) {
  ...
  if ((result=gapi_prepare(argv[1],0))!=0) {
    switch (result) {
    case -1:
      fprintf(stderr,"[%s]: file %s couln't be prepared\n",PROGNAME,argv[1]);
      exit (1);
      break;
    case -2:
    ...
    fprintf(stderr,"[%s]: file %s is beiing prepared\n",PROGNAME,argv[1]);
    exit(0);
  }
}
