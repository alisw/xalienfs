...
#  define DEBUGMSG(X,MSG) if(getenv(DEBUGENVNAME)!=NULL && atoi(getenv(DEBUGENVNAME)) >= X) { \
...
#  define DEBUGHEXDUMP(X,MSG,PTR,LENGTH) if (getenv(DEBUGENVNAME)!=NULL && atoi(getenv(DEBUGENVNAME)) >= X) {                                 \
                                           if (getenv("DEBUGFILE")!=NULL) {                                                                   \
                                             std::ofstream dbgstr(getenv("DEBUGFILE"),std::ios::app);                                         \
                                             dbgstr << "(" << DEBUGENVNAME <<"=" << X                                                         \
                                                    << "," << __FUNCTION__ << "): " MSG;                                                      \
                                             ...
                                             int counter=0;                                                                                   \
                                             for (unsigned char *debgi=(unsigned char*)PTR; debgi<((unsigned char*) PTR+(LENGTH)); debgi++) { \
