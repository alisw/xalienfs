...
void usage() {
      ...
      fprintf (stderr, "          \t              : 0  = File is online");
      fprintf (stderr, "          \t              : -1 = File is offline");
...
int main(int argc, char* argv[]) {
  ...
  if ((result=gapi_isonline(argv[1]))!=0) {
    switch (result) {
    case -1:
      fprintf(stderr,"[%s]: file %s is offline\n",PROGNAME,argv[1]);
      exit (1);
      break;
    case -2:
    ...
    fprintf(stderr,"[%s]: file %s is online\n",PROGNAME,argv[1]);
    exit(0);
  }
}
